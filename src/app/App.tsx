...
function AppContent() {
  ...
  const handleNavigation = (screen: AppScreen, data?: Property) => {
    ...
    } else if (!["detail", "chat"].includes(screen)) {
  ...
  const showBottomNav = role === "user" && ["home", "search", "map", "profile", "favorites", "myListings", "chat"].includes(currentScreen);
  ...
  const renderScreen = () => {
    ...
    switch (currentScreen) {
      ...
      case "adminDashboard":
        const isAdmin = ["superadmin", "admin", "moderator"].includes(role || "");
        return isAdmin ? <ScreenWrapper keyName="adminDashboard" animation={fadeProps}><AdminDashboardScreen onLogout={handleLogout} onNavigate={(s, d) => handleNavigation(s as AppScreen, d as Property)} /></ScreenWrapper> : null;
          ...
          <nav className="flex-1 space-y-2">
            {[
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all ${currentScreen === item.id
